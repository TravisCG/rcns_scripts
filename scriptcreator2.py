...
for i in exphash:
	...
	print('pdf("%s.pdf")' % i)
	print('ggplot(subset) + geom_violin(aes(x=contains_target_motif, y = peakscore)) + scale_x_discrete(labels = c("other", "%s")) + ggtitle("Comparin findpeak scores in four experiments") + ylab("Findpeak score") + xlab("Target motif")' % "+".join(exphash[i]))
	print('dev.off()')
